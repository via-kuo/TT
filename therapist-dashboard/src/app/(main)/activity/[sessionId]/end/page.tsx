...
export default function SessionEndPage() {
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#f5e6d3] px-[10%] pt-11 lg:pt-[47px] pb-6 flex flex-col gap-2">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              {/* 分數欄標題 */}
              {[1, 2, 3, 4].map((score) => (
                <th key={score} className="border-b border-r border-[#e8e8e8] last:border-r-0 py-4">
          ...
          <tbody>
            {CRITERIA.map((row, rowIdx) => (
              <tr key={row.name} className="border-b border-[#e8e8e8] last:border-b-0">
                {/* 列名 */}
                <td className="border-r border-[#e8e8e8] px-6 py-4 text-[15px] font-medium text-[#1a1a1a]">
                  {row.name}
                </td>
                {/* 各分數選項 */}
                {row.options.map((option, colIdx) => {
                  ...
                  return (
                    <td
                      ...
                      className={`border-r border-[#e8e8e8] last:border-r-0 px-4 py-4 text-[14px] text-center transition-colors ${
                        selected
